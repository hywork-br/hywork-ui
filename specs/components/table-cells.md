...
Semântica consistente de conteúdo, pessoa, status, data e número; ausência não vira zero.
...
Exports: `ContentCell`, `PersonCell`, `StatusCell`, `DateCell`, `NumberCell`. Conteúdo e ações são nomeados pelo consumidor; wrappers preservam a semântica nativa.

## API e defaults

ContentCell title/metadata/thumbnail; PersonCell name/src/description; StatusCell label/tone; DateCell value/locale; NumberCell value/locale/options. A fonte completa é `src/index.ts` e as declarações TypeScript do pacote; esta spec não amplia a API.
...
Draft em 2026-09-05. Owners: Hywork Product Design e Hywork Frontend. Justificativa estrutural: Semântica consistente de conteúdo, pessoa, status, data e número; ausência não vira zero. Zero consumidores comprovados; promoção depende dos pilotos de outubro. Contrato executável: `stories/QualityContracts.stories.tsx#CellsContract`.
